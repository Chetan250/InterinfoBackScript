...
public class RefreshCustomReportDao {
	...
	public ArrayList<CustomReportBean> getData() {
		...
		/*query = query + " where (refresh_date = '*' or refresh_date = '" + day
				+ "') and " + " (refresh_month = '*' or refresh_month = '"
				+ month + "') and " + " (year = '*' or year = '" + year + "')";*/
	...
	public void closeAll() {
		try {
			...
			System.out
					.println("connection close in refresh  Custom Report Dao");
		} catch (Exception e) {
	...
	public ArrayList<CustomReportBean> getSingleData() {
		ArrayList<CustomReportBean> objBeans = new ArrayList<CustomReportBean>();
		String query = "select a.* from custom_report a JOIN custom_report_run_now b ON a.report_id = b.report_id limit 1 ";
		CustomReportBean bean = null;
		try {
			stmt = conn.createStatement();
			rs = stmt.executeQuery(query);
			...
			System.out.println("Size of List in getSingleData() =" + objBeans.size());
		} catch (Exception e) {
	...
	public void deleteReportId(String reportId) {
		System.out.println("deleteReportId reportId");
		System.out.println(reportId);
		String query = "delete from custom_report_run_now where report_id = " + reportId;
		try {
			stmt = conn.createStatement();
